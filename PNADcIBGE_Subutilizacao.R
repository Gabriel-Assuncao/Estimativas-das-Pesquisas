...
pnadc_trimestral$variables <- transform(pnadc_trimestral$variables, Estrato_Geografico=as.factor(substr(x=Estrato, start=1, stop=4)))
utils::download.file(url="https://painel.ibge.gov.br/saibamais/files/Municipios_por_Estratos.csv", destfile=paste0(tempdir(),"/Municipios_por_Estratos.csv"))
municipios_estratos <- utils::read.csv(file=paste0(tempdir(),"/Municipios_por_Estratos.csv"), header=TRUE, sep=";", dec=",")
