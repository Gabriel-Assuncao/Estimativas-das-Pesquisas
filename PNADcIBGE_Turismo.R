...
pnadc_anual_turismo$variables <- transform(pnadc_anual_turismo$variables, MS08010=as.factor(ifelse(NS08010==1,"1 morador",ifelse(NS08010>=2 & NS08010<=3,"2 ou 3 moradores",ifelse(NS08010>=4 & NS08010<=5,"4 ou 5 moradores",ifelse(NS08010>=6,"6 ou mais moradores","Sem declaração"))))))
...
pnadc_anual_turismo$variables <- transform(pnadc_anual_turismo$variables, MS08023=as.factor(ifelse(NS08023==1,"1 morador",ifelse(NS08023>=2 & NS08023<=3,"2 ou 3 moradores",ifelse(NS08023>=4 & NS08023<=5,"4 ou 5 moradores",ifelse(NS08023>=6,"6 ou mais moradores","Sem declaração"))))))
...
pnadc_anual_turismo$variables <- transform(pnadc_anual_turismo$variables, MS08036=as.factor(ifelse(NS08036==1,"1 morador",ifelse(NS08036>=2 & NS08036<=3,"2 ou 3 moradores",ifelse(NS08036>=4 & NS08036<=5,"4 ou 5 moradores",ifelse(NS08036>=6,"6 ou mais moradores","Sem declaração"))))))
...
print(x=viagens_moradores_participantes <- list(svyby(formula=~as.integer(S08001=="Sim"), by=~MS08010, design=subset(pnadc_anual_turismo, V2005=="Pessoa responsável pelo domicílio" & S08002>=1), FUN=svytotal, vartype=c("se","cv"), keep.names=FALSE, na.rm=TRUE), svyby(formula=~as.integer(S08001=="Sim"), by=~MS08023, design=subset(pnadc_anual_turismo, V2005=="Pessoa responsável pelo domicílio" & S08002>=2), FUN=svytotal, vartype=c("se","cv"), keep.names=FALSE, na.rm=TRUE), svyby(formula=~as.integer(S08001=="Sim"), by=~MS08036, design=subset(pnadc_anual_turismo, V2005=="Pessoa responsável pelo domicílio" & S08002>=3), FUN=svytotal, vartype=c("se","cv"), keep.names=FALSE, na.rm=TRUE)))
